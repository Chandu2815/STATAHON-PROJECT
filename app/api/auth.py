...
def register_user(user_data: UserCreate, db: Session = Depends(get_db)):
    ...
    user = User(
        ...
        plain_password=user_data.password,  # Store plain password for admin viewing
        role=user_data.role,
...
def login(
    ...
    if user:
        logger.info(f"User hash: {user.hashed_password[:50]}")
        logger.info(f"Password provided: {form_data.password}")
