...
def login_start(payload: LoginStartRequest, request: Request, db: Session = Depends(get_db)):
    """Start login by validating credentials and requesting authenticator code"""
    
    ip_address = request.client.host if request.client else "unknown"
    
    # ===== SECURITY CHECK 1: Check if account is PERMANENTLY locked =====
    lockout_check = LoginSecurityService.check_account_lockout(db, payload.username)
    
    if lockout_check.get("is_permanent"):
        # Account is PERMANENTLY locked - admin unlock required
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={
                "error": "account_permanently_locked",
                "message": "Account has been permanently locked due to repeated failed attempts. Please contact your administrator to unlock your account.",
                "locked_by_admin": lockout_check.get("locked_by_admin", False),
                "attempt_count": lockout_check.get("attempt_count", 0)
            }
        )
    
    if lockout_check.get("is_locked"):
        # Account is temporarily locked - 30 minute lockout
        remaining_seconds = lockout_check.get("remaining_seconds", 1800)
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "locked_until": (datetime.utcnow() + timedelta(seconds=remaining_seconds)).isoformat()
            }
        )
    
    if lockout_check.get("is_cooldown"):
        # User is in 30-second cooldown between attempts
        remaining_seconds = lockout_check.get("remaining_cooldown", 30)
        attempt_count = lockout_check.get("attempt_count", 0)
        ...
        raise HTTPException(
            ...
            detail={
                "error": "rate_limited",
                "message": f"Too many failed attempts ({attempt_count}/5). Please wait {remaining_seconds} seconds before trying again",
                "remaining_seconds": remaining_seconds,
    ...
    if not user or not verify_password(payload.password, user.hashed_password):
        ...
        LoginSecurityService.record_failed_attempt(
            db,
            payload.username,
        ...
        # Get updated attempt info for response
        attempt_info = LoginSecurityService.get_failed_attempt_info(db, payload.username)
        ...
        raise HTTPException(
            ...
            detail={
                ...
                "attempts_remaining": max(0, attempt_info["max_attempts"] - attempt_info["attempt_count"])
            }
        )
    ...
    # Don't clear the failed attempts yet - only clear on successful OTP verification
...
def login_verify(payload: LoginVerifyRequest, request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        # OTP verification failed - record the failed attempt
        # We need to get the user from the challenge to record which user failed
        challenge_record = db.query(OtpChallenge).filter(
            OtpChallenge.challenge_id == payload.challenge_id
        ).first()
        
        if challenge_record and challenge_record.user_id:
            user = db.query(User).filter(User.id == challenge_record.user_id).first()
            if user:
                LoginSecurityService.record_failed_otp_attempt(
                    db,
                    user.username,
                    user.id,
                    ip_address
    ...
    # ===== OTP Verification Successful - Clear failed attempts =====
    LoginSecurityService.record_successful_login(db, user.username, user.id, ip_address)
